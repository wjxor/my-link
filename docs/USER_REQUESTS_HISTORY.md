...
### 2026-06-09 Request 23
- 헤더에 구글 소셜 로그인 기능 추가해줘.
- Firestore 경로 : users/{user.uid}/links로 개인화, 해당 경로의 유저 정보를 화면에 보여줘야 해.
- 로그인에 따른 화면도 구성해줘야 해. 만약 로그인을 안 했다면 로그인 이후에 사용할 수 있도록 설명 문구를 페이지에 보여줘야 해.

### 2026-06-09 Request 24
- 랜딩 페이지 가운데에도 로그인 버튼을 만들어주고 리디자인해줘

### 2026-06-09 Request 25
- 하단에 100% 무료 그런건 빼

### 2026-06-09 Request 26
- 로그인하고 우측상단에 프로필 아이콘이 있는데 드롭다운에 로그아웃을 넣으면 좋겠어.
- 프로필 오른쪽에 있는 로그아웃은 없애고

### 2026-06-09 Request 27
- 런타임 오류: Base UI: MenuGroupContext is missing. Menu group parts must be used within <Menu.Group> or <Menu.RadioGroup>. 해결

### 2026-06-09 Request 28
- 그리고 사이트가 자꾸 번역하겠냐고 떠

### 2026-06-09 Request 29
- 프로필 드롭다운에 정보가 더 있었으면 좋겠어. 어떤 정보가 있으면 좋을지 추천해줘

### 2026-06-09 Request 30
- 1, 2, 3번 추천 기능(이메일 요약, 링크 복사, 페이지 미리보기) 구현해줘

### 2026-06-09 Request 31
- 미리보기는 지금 404 뜨는 이유가 뭐야? 이유만 알려줘

### 2026-06-09 Request 32
- 마이페이지에 프로필 수정 기능 추가해줘.
- display name도 수정이 가능해야 하는데 혹시 이거 중복 확인도 가능해?

### 2026-06-09 Request 33
- 프로필 수정 버튼을 눌러봤는데 아무것도 안보여 (수정 버튼 시인성 개선)

### 2026-06-09 Request 34
- 프로필 수정버튼 눌렀을 때 나타나는 화면에서 텍스트박스 및 취소 버튼이 안보임 (스타일 누락 수정)

### 2026-06-09 Request 35
- 취소 버튼에 배경색 넣어줘

### 2026-06-09 Request 36
- `/[displayName]` 동적 라우팅 페이지 만들어줘.
- displayName으로 유저 검색
- 링크:users/{userId}/links
- username 없으면 404 페이지
- 헤더와 프로필 드롭다운에 개인별 페이지 이동을 추가해야 해.

### 2026-06-09 Request 37
- 3번 헤더 버튼 추가는 안해도 돼

### 2026-06-09 Request 38
- 헤더 상단에 이름없애주고 버튼 다시 추가해주는데 '내 페이지'로 해줘

### 2026-06-09 Request 39
- 내 페이지 바로가기 디자인을 버튼 primary로 해줘

### 2026-06-09 Request 40
- 상단부분(헤더부분)과 페이지의 색이 달라서 경계부분이 느껴지네 페이지 부분과 색깔 맞춰줘

### 2026-06-09 Request 41
- 내 페이지 버튼을 흰색배경에 검은색 글자로 해줘
...
- 작업할때 Task, Implement plan, walkthrough 는 자동으로 띄우거나 수정해주면 안될까? 너가 무슨 작업을 하는지 내가 보고 싶어

- 보안 규칙에 클릭 카운트는 누구나 수정이 가능한지 있어?

- 커밋해줘

## 2026-07-01: 랜딩 페이지 디자인 개선
**Request:** 마이링크 루트(/)에 랜딩 페이지 개선해줘. 랜딩 페이지에 들어가면 좋은 요소들을 현재 페이지 구조의 레벨에 맞게 재미있게 디자인해서 개선해줘. 반응형을 적용해야해

**Request:** 서버 열어줘

**Request:** 랜딩 페이지 중간에 구글 아이콘과 구글 계정으로 3초만에 시작하기 버튼에서 모바일 버전으로 보면 구글 아이콘이 왼쪽으로 튀어나간다 수정해줘

**Request:** 그리고 구글 아이콘과 글씨가 왼쪽으로 좀 쏠려있는 느낌이야

**Request:** 커밋해줘

**Request:** 푸쉬해
- 동적 OG 이미지 만들어줘. 모든 경로에 추가해주는데, 동적 경로랑 일반 경로만 분리하면 되겠어. 동적 경로는 예쁘게 디자인 해줘. 현재 디자인 시스템의 분위기를 고려해서 통일감 있는 오픈 그래프 이미지를 만들어줘. nextjs의 파일 시스템을 이용한 오픈 그래프야.

## 2026-07-01: SEO 메타데이터 전체 페이지 추가
**Request:** 모든 페이지에 메타데이터를 추가해줘. SEO 개선이 목적이야.
